...
async function test() {
  ...
  const sdk = new LSMSDK({
    configPath: path.join(__dirname, 'lsm-ygopro-database', 'labels.yaml'),
    databasePath: path.join(__dirname, 'lsm-ygopro-database', 'cards.cdb')
  });
...
②：这张卡进行战斗的伤害计算后才能发动。从卡组把1张「闪刀」卡送去墓地。, card_type: 效果, monster_type: 连接, attribute: 风, race: 机械, ocg_tcg: OCG|TCG, link: 1, atk: 1500, effect_type: 送去墓地, series: 闪刀`;
  ...
  let result;
  try {
    result = await sdk.searchByKeywords(keywords);
  } catch (e) {
    console.log('搜索失败:', e);
    return;
  }
